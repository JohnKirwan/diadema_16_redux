...
```{r}
plot.circular(df$heading_rad[df$type=="unmissable"])
df <- df[df$type != "unmissable",]
df$unmissable <- NULL
```
...
df %>% group_by(type) %>%
  ...
  mutate(in.sext = ifelse(abs(heading_rad) < pi/6, 1, 0)) %>% # for data in radians -pi to pi
  mutate(in.oct  = ifelse(abs(heading_rad) < pi/8, 1, 0)) %>% # for data in radians -pi to pi
  group_by(arc,type) %>%                # condition name
...
circ_fun_DoG %>%
  ...
  geom_line(aes(y=quad.prop), color="blue")    +    ## data in target octant
   ...
   ylim(0,0.55) + facet_wrap(vars(type),scales = "free_x")
```
...
props %>%
  ggplot(aes(x=T,y=p)) + geom_point() + theme_classic() + geom_hline(yintercept = 0.1667,color='red') + facet_wrap(vars(type),scales = "free_x")
```
...
lapse      <- rbeta(N,10,10)       # Lapse is wide range - should not overlap base rate
lapse[lapse > 0.7] <- 0.7
threshold  <- rnorm(N,0,3)# + sigma    # Threshold must be > 0 and can be > 1.
width      <- rnorm(N,0,5)# + sigma   # Width must be > 0 and can be > 1.
...
for ( i  in 1:N ) curve (
        base[i] +
          (1 - base[i] - lapse[i]) *   
          plogis(4.39*(	x - threshold[i]	)	/ ( (width[i]) ) ),
   from=-3, to = 3, add = TRUE, col = "black")
```
...
```{r}
get_prior(psych.s.formula, data=df)
```
...
psych.s.priors  <- c(
         ...
         prior(normal(0,3),          nlpar= 'threshold',  class = 'b'  ),
	       prior(normal(0,5),          nlpar = 'width',     class = 'b' , lb=0 )
         )
```
...
```{r Main model}
psych_s_fit_pr  <- brm(psych.s.formula, data = df,
                  family = bernoulli("identity"), iter = iter,
                  chains = chains, prior = psych.s.priors, sample_prior = "only",
                  control = list(adapt_delta=0.999))
```
...
```{r Main model}
psych_s_fit  <- brm(psych.s.formula, data = df,
                  family = bernoulli("identity"), iter = iter,
                  chains = chains, prior = psych.s.priors, 
                  control = list(adapt_delta=0.999))
```
