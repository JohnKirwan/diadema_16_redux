...
lapse[lapse > 0.7] <- 0.7
threshold  <- rgamma(N,2,1.5)# + sigma    # Threshold must be > 0 and can be > 1.
width      <- rgamma(N,2,1.5)# + sigma   # Width must be > 0 and can be > 1.
...
res.psych.formula <- bf(
  success ~ base + 
            (1 - base - lapse) * inv_logit( 4.39*(	T - threshold	)	/ width ),
            base      ~ 1, 
            lapse     ~ 1,# + (1|locale),
            threshold ~ type,
            width     ~ type,  nl = TRUE)
...
```{r}
conditions <-  make_conditions(df, vars = c("type"))

p <- plot(conditional_effects(res_psych_fit, effects = "T", theme = "classic",
                         conditions = conditions,spaghetti=TRUE),points=TRUE)
#p$T + theme_classic()
```
...
```{r}
res_psych_fit <- add_criterion(res_psych_fit, criterion = "loo")
res_empty_fit <- add_criterion(res_empty_fit, criterion = "loo")
...
bar_post <- res.psych.smpl$b_threshold_Intercept*max(df$arc)
DoG_post <- bar_post + (res.psych.smpl$b_threshold_typeDoG * max(df$arc))
...
```{r}
bayesplot::pp_check(res_psych_fit,nsamples=400, type="bars",) + theme_classic() + theme(legend.position='none')
```


```{r}
bayesplot::pp_check(res_psych_fit, type = "rootogram", nsamples = 100, style = "standing", prob=0.9) +   theme_classic()
```
...
base[base > 0.2] <- 0.2
lapse      <- rbeta(N,10,10)       # Lapse is wide range - should not overlap base rate
lapse[lapse > 0.79] <- 0.79
threshold  <- rnorm(N,0,5)# + sigma    # Threshold must be > 0 and can be > 1.
width      <- rgamma(N,2,1)# + sigma   # Width must be > 0 and can be > 1.
                                   # Varying intercepts base

plot( NULL, xlim = c(-2,3), ylim = c(0,0.8) ,
      xlab = "Scaled stimulus strength", ylab = "Proportion correct response" )
abline (h=1/6, lty=2)
for ( i  in 1:N ) curve (
        base[i] +
          (1 - base[i] - lapse[i]) *   
          plogis(4.39*(	x - threshold[i]	)	/ ( (width[i]) ) ),
   from=-2, to = 3, add = TRUE, col = "black")
```
...
```{r}
res.psych.formula.s <- bf(
  success ~ base + 
            (1 - base - lapse) * inv_logit( 4.39*(	scaled_arc - threshold	)	/ width ),
            base      ~ 1, 
            lapse     ~ 1,# + (1|locale),
            threshold ~ type,
            width     ~ type,  nl = TRUE)
...
```{r}
get_prior(res.psych.formula.s, data=df)
```


```{r}
res.psych.priors.s  <- c(
         prior(beta(5,50),          nlpar = "base",   lb=0, ub= 0.2),
         prior(beta(10,10),         nlpar = "lapse" , lb=0,ub= 0.79   ),
         prior(normal(1,2),         nlpar= 'threshold',  class = 'b'  ),
	       prior(gamma(2,1),          nlpar = 'width',     class = 'b'  )
         )
```
...
```{r Main model}
res_psych_fit_pr.s  <- brm(res.psych.formula.s, data = df,
                  family = bernoulli("identity"), iter = iter,
                  chains = chains, prior = res.psych.priors.s, sample_prior = "only",
                  control = list(adapt_delta=0.999))
```
...
p <- conditional_effects(res_psych_fit_pr.s, effects = "scaled_arc",
                         conditions = conditions,
                      #   int_conditions = list(scaled_arc = quantile),
                         prob = 0.95)  # ,spaghetti=TRUE,points=TRUE
plot(p)[[1]] + scale_x_continuous(limits=c(-2,2))
```


```{r Main model}
res_psych_fit_s  <- brm(res.psych.formula.s, data = df,
                  family = bernoulli("identity"), iter = iter,
                  chains = chains, prior = res.psych.priors.s, 
...
```{r}
summary(res_psych_fit_s)
```

The parameters are unimodal and the chains converged, resembling *hairy caterpillars.* Lapse is skewed, on account of the constraint imposed and because low values are perfectly plausible.

```{r}
plot(res_psych_fit_s,ask=FALSE)
```
...
unscale <- function(){
  scales::trans_new(
    "unscale",
    function(x) arc_center + x * arc_scale,
    function(x) scale(x) )
}

descale <- function(x){ 
  round(
  arc_center + x * arc_scale,
  0)
  }

conditions <-  make_conditions(df, vars = c("type"))

p <- conditional_effects(res_psych_fit_s, effects = "scaled_arc",
                         conditions = conditions,spaghetti=TRUE,points=TRUE)
```
...
```{r}
p[[1]][5] <- arc_center + (p[[1]][5] * arc_scale)
p[[1]][1] <- arc_center + (p[[1]][1] * arc_scale)
...
```{r Extract the samples}
res.psych.smpl <- posterior_samples(res_psych_fit_s)

bar_post <- res.psych.smpl$b_threshold_Intercept*descale(df$arc)
DoG_post <- bar_post + (res.psych.smpl$b_threshold_typeDoG * descale(df$arc))
thresh_df <- tibble(bar=bar_post,DoG=DoG_post)
thresh_df <- gather(thresh_df,key="type",value = "threshold")
