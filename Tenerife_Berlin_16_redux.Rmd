...
df %>% as_tibble %>%                 # dataframe name
  ...
  group_by(arc,locale,DoG) %>%                # condition name
...
source('Cplot2_rad.R')
par(mfrow = c(3,ceiling( nlevels(as.factor(df$T[df$DoG==1])) /3)));
par(mar = c(0.5, 0.5, 0.5, 0.5) + 0.1) # bottom, left, top, right
for(i in levels(as.factor(df$arc[df$DoG==1])) ){
    rad_plot(df$heading_rad[df$arc[df$DoG==1]==i],0.06)
...
```{r Circular plots, message=FALSE}
source('Cplot2_rad.R')
for(i in levels(as.factor(df$arc[df$DoG==1 & df$pattern=="dog 40"])) ){
    rad_plot(df$heading_rad[df$arc[df$DoG==1]==i],0.06)
    text(0,.6,as.character(round(as.numeric(i),2)))
 }
```
...
```{r Circular plots, message=FALSE}
source('Cplot2_rad.R')
par(mfrow = c(3,ceiling( nlevels(as.factor(df$T[df$DoG==0])) /3)));
par(mar = c(0.5, 0.5, 0.5, 0.5) + 0.1) # bottom, left, top, right
for(i in levels(as.factor(df$T[df$DoG==0])) ){
    rad_plot(df$heading_rad[df$T[df$DoG==0]==i],0.06)
    text(0,.6,as.character(round(as.numeric(i),2)))
 }
```
...
```{r}
res.psych.priors  <- c(
         prior(normal(0.1,0.01),     nlpar = "base",   lb=0,ub= 0.2),
         prior(beta(15,15),          nlpar = "lapse" , lb=0,ub= 0.75   ),
...
res.psych.formula <- bf(success ~
                      base +
                      (1 - base - lapse) *
                      inv_logit( 4.39*(	T*DoG - threshold	)	/  width  ),
