...
df %>% mutate(H = d2r(heading)) -> df
df %>% mutate(H = unlist(vonMapease(H))) -> df # make headings in rad

df %>% mutate(success = 0) -> df
for(i in 1:length(df$H)){           ### get tote of each using one fifth of the circle
  if(is.na(df$H[i]) == TRUE) {df$success[i] <- 0}
  else if( df$H[i] >   pi/6) {df$success[i] <- 0}    ## pi/5 beforehand
  else if( df$H[i] <= -pi/6 ){df$success[i] <- 0}  ## -pi/5 beforehand
  else{    df$success[i] <- 1}  }
```
...
circ_fun_res %>%
  ...
  geom_line(aes(y=quad.prop), color="blue")    +    ## data in target octant
   ...
   geom_bin2d(data=df2,aes(y = success, x = 180*T/pi)) + # counts
   ylim(0.1,0.55)
```
...
df$rad <- as.circular(sapply(df$theta,centrerad))
...
```{r}
rayleigh.test(df$theta[df$pattern=="bar 40"])
```

```{r}
rayleigh.test(df$theta[df$pattern=="bar 40"],mu=0)
```
