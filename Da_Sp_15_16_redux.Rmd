---
title: "Spatial Resolution in Diadema africanum and Strongylocentrotus purpuratus"
...
# Spine and sight: a psychometric model of vision in *Diadema africanum* and *Strongylocentrotus purpuratus*
...
props %>%
  ggplot(aes(x=T,y=p)) + geom_point() + theme_classic() + geom_hline(yintercept = 0.1667,color='red') + facet_wrap(vars(species,type),scales = "free")
```
...
par(mfrow = c(rowz,ceiling( nlevels(as.factor(df$arc[df$type==typ & df$species==spp])) /rowz)));
par(mar = c(0.5, 0.5, 0.5, 0.5) + 0.1) # bottom, left, top, right
for(i in levels(as.factor(df$arc[df$type==typ & df$species==spp])) ){
    rad_plot(df$heading_rad[df$arc == i & df$type==typ & df$species==spp],0.06)
    text(0,.6,as.character(round(as.numeric(i),2)))
 }
```

DoG stimuli - *D. africanum*
...
lapse[lapse > 0.75] <- 0.75
threshold  <- rnorm(N,2,3)# + sigma   # Threshold must be > 0 and can be > 1.
width      <- rnorm(N,3,2)# + sigma   # Width must be > 0 and can be > 1.
...
```{r Specify priors}
res.psych.priors  <- c(
         prior(beta(5,50),           nlpar = "base",   lb=0, ub= 0.25 ),
         prior(beta(10,10),          nlpar = "lapse" , lb=0, ub= 0.75 ),
         prior(normal(2,3),        nlpar= 'threshold',  class = 'b' ),
	       prior(normal(3,2),          nlpar = 'width',  lb=0, class = 'b' )
...
conditional_effects(res_psych_fit_pr, spaghetti=FALSE,
...
thresh_df <- tibble(
  bar_Da=res.psych.smpl$b_threshold_Intercept,
  DoG_Da=res.psych.smpl$b_threshold_Intercept + res.psych.smpl$b_threshold_typeDoG,
  bar_Sp=res.psych.smpl$b_threshold_Intercept + res.psych.smpl$b_width_speciesSp,
  DoG_Sp=res.psych.smpl$b_threshold_Intercept + res.psych.smpl$b_width_speciesSp +
         res.psych.smpl$b_width_typeDoG + res.psych.smpl$`b_threshold_speciesSp:typeDoG`
  )
  

thresh_df <- gather(thresh_df,key="type",value = "scaled_threshold")
thresh_df %>% mutate(threshold = arc_center + scaled_threshold * arc_scale) -> thresh_df

thresh_df %>% group_by(type) %>%
  ggplot(aes(x=threshold,group=type,color=type)) + geom_density() +xlim(0,150) + theme(legend.position = c(.9,.8))
```
...
thresh_df %>% group_by(type) %>%
  summarise(
    ...
    PI_90_lo   = round(PI(threshold,prob=.9),0)[1],
    PI_90_hi   = round(PI(threshold,prob=.9),0)[2],
    HPDI_90_lo = round(HPDI(threshold,prob=.9),0)[1],
    HPDI_90_hi = round(HPDI(threshold,prob=.9),0)[2],
  ) -> thresh_sum
